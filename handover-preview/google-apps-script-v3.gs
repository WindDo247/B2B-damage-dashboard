function setupEntireWorkbook() {
  ...
  // ==========================================
  function getOrCreateSheet(name, index) {
    var existing = ss.getSheetByName(name);
    if (existing) {
      existing.clear();
      existing.getRange('A:Z').setDataValidation(null);
      existing.clearConditionalFormatRules();
      if (existing.getFilter()) { existing.getFilter().remove(); }
      return existing;
    }
  ...
  // ==========================================
  // Use first sheet and rename it
  var sheet1 = ss.getSheetByName('Phan Cong PIC');
  if (!sheet1) {
    sheet1 = ss.getSheets()[0];
    sheet1.setName('Phan Cong PIC');
  }
  sheet1.clear();
  sheet1.getRange('A:Z').setDataValidation(null);
  ...
  // Data — set values FIRST, then add validation AFTER
  var data1 = [
  ...
  // Data validation AFTER data is set — values must match exactly
  ...
  // SHEET 2: LIEN HE PIC
  // ==========================================
  var sheet2 = getOrCreateSheet('Lien He PIC', 1);
  ...
  // ==========================================
  var sheet3 = getOrCreateSheet('Quy Trinh Escalation', 2);
  ...
  // ==========================================
  var validNames = ['Phan Cong PIC', 'Lien He PIC', 'Quy Trinh Escalation'];
  var allSheets = ss.getSheets();
  for (var s = allSheets.length - 1; s >= 0; s--) {
    var sName = allSheets[s].getName();
    if (validNames.indexOf(sName) === -1 && allSheets.length > 3) {
